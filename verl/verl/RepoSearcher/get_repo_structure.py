...
def get_project_structure_from_scratch(
    ...
    repo_folder = f"{repo_playground}/{repo_to_top_folder(repo_name)}"
    ...
    subprocess.run(
        [
            "cp", "-r",f"{repo_folder}", f"{repo_playground}",
        ],
        check=True,
    )
    ...
    # breakpoint()
    # goto_last_commit(f"{repo_playground}/{get_top_folder(repo_name)}", commit_id)
