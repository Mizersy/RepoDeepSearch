                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    ...
                    if self._function_call_parser and self._function_call_parser.has_tool_call(content):
                        ...
                        if len(parsed_tool_calls) > 0:
                            ...
                            # break
                            _req.add_tool_response_messages(self.tokenizer, ["""wrong tool call format! For each function call, return a json object with function name and arguments within <tool_call></tool_call> XML tags:
For example:
<tool_call>
...
Note: all the arguments must be filled in."""])
        ...
        if not finish_reason_type == FinishReasonTypeEnum.LENGTH:
            ...
            if len(final_loc_prompt) < _req.max_model_len - len(_req.input_ids) - 1:
                _req.add_tool_response_messages(self.tokenizer, [final_loc_prompt])
                output = await self._handle_engine_call(_req, do_sample, is_validate, **kwargs)
                content = output["text"]
                _req.add_assistant_message(self.tokenizer, content)
    ...
    async def _handle_engine_call(self, _req: AsyncRolloutRequest, do_sample: bool, is_validate: bool, override_n: bool = True, **kwargs) -> dict:
        ...
        if max_new_tokens <= 0:
            print(f"[SGLangRollout] max_new_tokens is less than 0, max_new_tokens: {max_new_tokens}, max_model_len: {self.config.max_model_len}, len(generation_prompt_ids): {len(generation_prompt_ids)}")
            return {"text": "max_new_tokens is less than 0"}
        # Create request-level sampling parameters
        request_sampling_params = self.sampling_params.copy()
        if not do_sample:
            request_sampling_params.update(
        ...
        if "n" not in request_sampling_params or (request_sampling_params["n"] > 1 and override_n):  # group size is supported in preprocess
