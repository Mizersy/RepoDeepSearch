...
class GetCodeOfClassTool(BaseTool):
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        _tool_schema = OpenAIFunctionToolSchema.model_validate({
            ...
            "function": {
                ...
                "description": "Get the code of a specified class in the given file and python project.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "file_name": {
        ...
        logger.info(f"Initialized GetCodeOfClassFunctionTool with config: {config}")
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> Tuple[str, float, dict]:
        ...
        from verl.repoSearcher.fl.FL_tools import get_code_of_class
        try:
            search_result = await self.execution_pool.execute.remote(get_code_of_class,file_name, class_name, swe_instance_id, repo, base_commit)
            # search_result = await get_code_of_class(file_name, class_name, swe_instance_id)
            # print(f"[GetCodeOfClassFunctionTool] result: {search_result}")
            search_result_response = f"""
Here is a result of function call get_code_of_class({file_name}, {class_name}).
<code>
{search_result}
</code>
...
"""
            ...
            logger.error(f"[GetCodeOfClassFunctionTool] Execution failed: {e}")
