...
from agentless.util.preprocess_data import (
    ...
    get_repo_structure,
    line_wrap_content,
    transfer_arb_locs_to_locs, get_repo_files,
)
...
def process_loc(loc, args, swe_bench_data, prev_o, write_lock=None):
    ...
    if args.line_level or args.function_level:
        file_contents = dict()
    ...
    if args.loc_key in loc:
        file_to_edit_locs_all = loc[args.loc_key]
...
def repair(args):
    ...
    swe_bench_data = load_dataset("princeton-nlp/SWE-bench_Verified")["test"]
        ...
        if args.select_id == -1:
            ...
            try:
                ...
                for i, tmp_pred_file in enumerate(pred_files):
                    ...
                    if (
                        args.loc_key in loc
                        and tmp_pred_file in loc[args.loc_key]
                    ):
                        line_locs, context_intervals = transfer_arb_locs_to_locs(
                            loc[args.loc_key][tmp_pred_file],
                            None,
                            loc["found_files"][i],
                            args.context_window,
                            args.loc_interval,
...
def process_sample(i, args):
    """处理单个样本的逻辑"""
    # 为每个样本生成不同的输出文件名
    output_file = args.raw_output_file.replace(".jsonl", f"_{i}_processed.jsonl")
    args_copy = argparse.Namespace(**vars(args))  # 深拷贝 args，确保线程安全
    args_copy.output_file = output_file
    args_copy.select_id = i
    post_process_repair(args_copy)


def parallel_post_process(args):
    """
    多进程处理每个样本的 post_process_repair。
    """
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.num_threads) as executor:
        futures = [executor.submit(process_sample, i, args) for i in range(args.max_samples)]
        for future in concurrent.futures.as_completed(futures):
            try:
                future.result()  # 捕获并打印异常（如果有）
            except Exception as exc:
                print(f"处理样本时发生错误: {exc}")
...
def main():
    ...
    assert args.function_level or args.line_level, "Must specify one of function_level or line_level"
    if args.line_level:
        args.loc_key = "found_edit_locs"
    else:
        args.loc_key = "found_related_locs"
