        ...
        for _, root in enumerate(roots):
            ...
            for patch in patches[:]:
                if args.regression:
                    ...
                    regression_test_result = len(matching_results[0].get("regression", [0] * 10000)) if matching_results else len([0] * 10000)
                else:
...
def get_all_patches(instance_id, num_samples, deduplicate) -> list[tuple[int, Any]]:
...
def get_all_patches_num(instance_id, num_samples, deduplicate) -> list[tuple[int, Any, int]]:
        ...
        for instance_id in execution_results:
            ...
            if not patch_ids:
                ...
                if not all([x.strip() == "" for x in raw_patches]) and not all(
                    ...
                    maj_selected_id = random.choice([
                        i for i in valid_indices
                        if vote[patch_keys[i]] == max(vote.values())  # 过滤出平票的补丁
                    ])
                    ...
                    result = {
                        ...
                        "regression_test_result": sample["regression_test_result"],
                        "reproduction_test_result": sample["reproduction_test_result"],
                    }
                else:
...
def normalize_patches(args):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        futures = [
            executor.submit(process_patch, output_folder, i)
...
def process_patch(output_folder, i):
    output_file = output_folder / f"output_{i}_normalized.jsonl"
    ...
    for d in patches:
        try:
            ...
            d["normalized_patch"] = d["model_patch"]
