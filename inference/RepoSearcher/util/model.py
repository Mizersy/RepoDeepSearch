...
from repoSearcher.util.api_requests import create_chatgpt_config, request_chatgpt_engine, create_anthropic_config, request_anthropic_engine, send_request_devgpt, send_request_openapi, chat_vllm_server
...
class OpenAIChatDecoder(DecoderBase):
    ...
    def codegen(
        ...
        # ret = chat_vllm_server(model_name=self.name, messages=message, temperature=self.temperature, max_tokens=self.max_new_tokens, top_p=1)
        if ret:
...
class AnomyDecoder(DecoderBase):
    def __init__(self, name: str, logger, **kwargs) -> None:
        super().__init__(name, logger, **kwargs)
    ...
    def codegen(
        ...
        for _ in range(num_samples):
            ...
            if self.name in ["aws_claude35_sdk_sonnet_v2", "aws_sdk_claude37_sonnet"]:
                ret = send_request_devgpt(model=self.name, messages=messages, temperature=self.temperature, max_tokens=self.max_new_tokens)
                response = ret["choices"][0]["message"]["content"] if ret else ""
            elif self.name == "volc-deepseek-r1":
                ret = send_request_openapi(model=self.name, messages=messages, temperature=self.temperature, max_tokens=self.max_new_tokens)
