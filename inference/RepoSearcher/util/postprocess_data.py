...
def extract_func_locs_for_files(locs, file_names, keep_old_order=False):
    if keep_old_order:
        ...
        for line in loc.splitlines():
            if line.strip().endswith(".py"):
                ...
                if current_file_name in file_names:
                    new_file_list.append(current_file_name) if current_file_name not in new_file_list else None
                    if current_file_name not in results:
                        results[current_file_name] = []
                    results[current_file_name].append(line)
                else:
                    new_file_list.append(current_file_name)
                    if current_file_name not in results:
                        results[current_file_name] = []
                    results[current_file_name].append(line)
...
""".strip()
    ...
    from RepoSearcher.util.preprocess_data import line_wrap_content
